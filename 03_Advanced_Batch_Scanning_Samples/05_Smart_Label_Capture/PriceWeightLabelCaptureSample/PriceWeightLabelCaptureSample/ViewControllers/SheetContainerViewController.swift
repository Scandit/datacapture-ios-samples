...
extension SheetContainerViewController {
    ...
    private func configureForExpandedStateAllowed() {
        if isExpandedStateAllowed, let superview = view.superview {
            sameHeightConstraint = view.heightAnchor.constraint(equalTo: superview.heightAnchor)
    ...
    private func onDrag(sender: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            dragPositionConstraint = view.topAnchor.constraint(
        ...
        case .changed:
            guard let dragStartState else { return }
    ...
    func present(in viewController: UIViewController, presentingView: UIView? = nil, animated: Bool = true) {
        ...
        guard let superview = presentingView ?? viewController.view else {
            return
        }
