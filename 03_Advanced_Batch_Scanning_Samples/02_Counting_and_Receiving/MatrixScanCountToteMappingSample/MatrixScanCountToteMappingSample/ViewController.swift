...
class ViewController: UIViewController {
    ...
    private func createBarcodeCountView() {
        ...
        barcodeCountView = BarcodeCountView(
            forMappingWithFrame: view.bounds,
            context: context,
...
extension ViewController: BarcodeCountListener {
    func barcodeCount(
        ...
        guard session.recognizedBarcodes.count > 0,
            let grid = session.spatialMap(withExpectedNumberOfRows: 4, expectedNumberOfColumns: 2)
        else {
...
extension ViewController: BarcodeSpatialGridEditorViewDelegate {
    func barcodeSpatialGridEditorView(
        _ view: BarcodeSpatialGridEditorView,
