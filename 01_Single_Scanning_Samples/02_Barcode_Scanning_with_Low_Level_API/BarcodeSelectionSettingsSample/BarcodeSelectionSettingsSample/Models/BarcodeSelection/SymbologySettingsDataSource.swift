...
class SymbologySettingsDataSource: DataSource {
    ...
    private var supportedExtensions: Set<String> {
        symbologyDescription.supportedExtensions.filter({ !$0.hasSuffix("add_on") })
    }
    ...
    var activeSymbolCountRange: Section {
        let currentMin =
            self.symbologySettings.activeSymbolCounts.minimum ?? symbologyDescription.activeSymbolCountRange.minimum
        let currentMax =
            self.symbologySettings.activeSymbolCounts.maximum ?? symbologyDescription.activeSymbolCountRange.maximum
