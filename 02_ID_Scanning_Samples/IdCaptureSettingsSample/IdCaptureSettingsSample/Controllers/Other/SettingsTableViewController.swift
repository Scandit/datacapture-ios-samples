...
class SettingsTableViewController: UITableViewController, DataSourceDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = cell as? TextEditCell, let value = row.getValue?() as? String {
            cell.delegate = self
            cell.value = value
        }
    ...
    func presentTextEdit(title: String?, currentValue: String, completion: @escaping (String) -> Void) {
        ...
        let saveAction = UIAlertAction(title: "Save", style: .default) { _ in
            guard let textFields = popup.textFields else { return }
            completion(textFields[0].text.valueOrEmpty)
        }
