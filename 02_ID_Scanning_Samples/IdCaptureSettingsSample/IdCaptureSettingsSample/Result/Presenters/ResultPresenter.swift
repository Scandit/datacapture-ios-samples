    ...
    static func getCommonRows(for capturedId: CapturedId) -> [CellProvider] {
        ...
        [
            SimpleTextCellProvider(
                value: capturedId.document?.documentType.description ?? "<nil>",
                title: "Document Type"
            ),
            SimpleTextCellProvider(
                value: capturedId.document?.subtypeDescription ?? "<nil>",
                title: "Subtype"
            ),
            ...
            SimpleTextCellProvider(
                value: capturedId.documentAdditionalNumber.valueOrNil,
...
struct CombinedResultPresenter: ResultPresenter {
    ...
    init(capturedId: CapturedId, presenters: [ResultPresenter]) {
        rows =
            Self.getCommonRows(for: capturedId) + presenters.flatMap({ $0.rows }) + Self.getImageRows(for: capturedId)
    }
}
