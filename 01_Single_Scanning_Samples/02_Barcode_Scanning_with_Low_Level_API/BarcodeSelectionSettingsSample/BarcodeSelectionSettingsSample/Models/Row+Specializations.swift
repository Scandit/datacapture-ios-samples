...
extension Row {
    static func valueWithUnit(
        ...
        Row(
            ...
            didSelect: { [weak dataSourceDelegate] row, _ in
                dataSourceDelegate?.getFloatWithUnit(
                    title: row.title,
    ...
    static func choice<Choice: CustomStringConvertible>(
        title: String,
        options: [Choice],
        ...
        Row(
            ...
            didSelect: { [weak dataSourceDelegate] row, _ in
                dataSourceDelegate?.presentChoice(
                    title: row.title,
                    options: options,
                    chosen: getValue(),
    ...
    static func symbology(
        getValue: @escaping (() -> SymbologySettings),
        didChangeValue: @escaping ((SymbologySettings) -> Void),
        dataSourceDelegate: DataSourceDelegate?
    ) -> Row {
        Row(
            title: getValue().symbology.readableName,
            kind: .symbology,
            getValue: getValue,
            didChangeValue: { [weak dataSourceDelegate] value in
                didChangeValue(value)
            ...
            didSelect: { [weak dataSourceDelegate] row, _ in
                guard let value = row.getValue?() as? SymbologySettings else { return }
                dataSourceDelegate?.presentSymbologySettings(
