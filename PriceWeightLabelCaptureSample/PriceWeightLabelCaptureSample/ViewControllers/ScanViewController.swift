...
class ScanViewController: UIViewController {
    ...
    private func showScanResults() {
        guard scanResultsViewController == nil else { return }
        let viewController = storyboard!.instantiateViewController(withIdentifier: "ScanResults")
        scanResultsViewController = viewController as? ScanResultsViewController
        scanResultsViewController!.delegate = self
        scanResultsViewController!.scannedItems = scannedItems
        scanResultsSheetViewController = SheetContainerViewController(content: scanResultsViewController!)
        scanResultsSheetViewController!.delegate = self
        scanResultsSheetViewController!.present(in: self, presentingView: containerView)
    }
    ...
    private func showEditMissingInfo(for item: Item) {
        guard editMissingInfoViewController == nil else { return }
        let viewController = storyboard!.instantiateViewController(withIdentifier: "EditMissingInfo")
        editMissingInfoViewController = viewController as? EditMissingInfoViewController
        editMissingInfoViewController!.delegate = self
        editMissingInfoViewController!.item = item
        editMissingInfoSheetViewController = SheetContainerViewController(content: editMissingInfoViewController!)
        editMissingInfoSheetViewController?.isExpandedStateAllowed = false
        editMissingInfoSheetViewController!.delegate = self
        editMissingInfoSheetViewController!.present(in: self, presentingView: containerView)
...
extension ScanViewController {
    ...
    private var regularItemLabelDefinition: LabelDefinition {
        let barcodeField = CustomBarcode(name: Field.barcode.rawValue,
